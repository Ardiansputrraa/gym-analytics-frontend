...
import { NutritionType, NutritionEntry, UpdateNutritionEntryInput } from '@/types/nutrition.types';
import { Utensils, Droplets, X, Edit3, Clock, Flame, Zap, PieChart } from 'lucide-react';

interface EditFoodModalProps {
  isOpen: boolean;
  entry: NutritionEntry | null;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateNutritionEntryInput) => Promise<void>;
  isSubmitting?: boolean;
}

export function EditFoodModal({ isOpen, entry, onClose, onSubmit, isSubmitting }: EditFoodModalProps) {
  ...
  useEffect(() => {
    if (entry) {
      ...
      setUnit(entry.unit || (entry.type === 'DRINK' ? 'ml' : 'porsi'));
      setProteinG(entry.proteinG !== null && entry.proteinG !== undefined ? entry.proteinG : '');
      setCarbsG(entry.carbsG !== null && entry.carbsG !== undefined ? entry.carbsG : '');
      setFatG(entry.fatG !== null && entry.fatG !== undefined ? entry.fatG : '');
      setWaterMl(entry.waterMl !== null && entry.waterMl !== undefined ? entry.waterMl : '');
      ...
      if (entry.consumedAt) {
        ...
        const parts = formatter.format(d).replace('.', ':').split(':');
        if (parts.length === 2) {
          setHourStr(parts[0]);
          setMinuteStr(parts[1]);
        }
      }
    }
  }, [entry]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let consumedDate: Date = entry.consumedAt ? new Date(entry.consumedAt) : new Date();
    const h = parseInt(hourStr, 10);
    const m = parseInt(minuteStr, 10);
    if (!isNaN(h) && !isNaN(m)) {
      consumedDate.setHours(h, m, 0, 0);
    }
    ...
    const payload: UpdateNutritionEntryInput = {
      type,
      name: name.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
      <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold font-[var(--font-display)] text-[var(--text-primary)]">
                Edit Catatan Nutrisi
              </h3>
              <p className="text-xs text-[var(--text-secondary)]">
                Perbarui data makanan atau minuman yang tercatat
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2 space-y-1">
              ...
              <input
                ...
                className="w-full h-10 px-3 rounded-xl bg-[var(--bg-base)] border border-[var(--border-default)] text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"
              />
            </div>
          ...
          {/* Notes */}
          <div className="space-y-1">
            <label className="text-xs font-semibold text-[var(--text-secondary)]">Catatan</label>
            <input
              type="text"
              value={notes}
          ...
          <div className="pt-3 border-t border-[var(--border-default)] flex items-center justify-end gap-3">
            ...
            <Button type="submit" variant="primary" size="md" disabled={isSubmitting || !name.trim()}>
              {isSubmitting ? 'Menyimpan...' : 'Perbarui Catatan'}
            </Button>
          </div>
        </form>
      </div>
